    ...
    protected String checkUser(User user, boolean create) {
        ...
        if (create) {
            ...
            if (Strings.isBlank(user.getPassword()))
                return "密码不能为空";
            if (0 >= dao.count(User.class, Cnd.where("name", "=", user.getName()))) {
                return "用户名已经存在";
            }
        }
        else {
...
//        user.setUpdateTime(new Date());// 设置正确的更新时间
    ...
    @At
    public Object delete(@Param("id")int id, @Attr("UserID")int me) {
        if (me == id) {
            return new NutMap().setv("ok", false).setv("msg", "不能删除自己!!");
        }
